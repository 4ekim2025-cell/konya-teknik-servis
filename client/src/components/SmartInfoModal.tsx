...
export function SmartInfoModal() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      const focusable = dialogRef.current?.querySelectorAll<HTMLElement>("button:not(:disabled), input, select, a[href]");
  ...
  useEffect(() => {
    const openFromQuickRequest = () => { setStep(1); setAttempted(false); setIsSending(false); setOpen(true); };
  ...
  const effectiveDevice = device === "Diğer" ? otherDevice.trim() : device;
  const effectiveSymptom = symptom === "Diğer" ? otherSymptom.trim() : symptom;
  const validDetails = Boolean(effectiveDevice && effectiveSymptom && districts.includes(district));
  const canContinue = step === 1 ? Boolean(effectiveDevice) : step === 2 ? Boolean(effectiveSymptom) : validDetails;
  ...
  function sendWhatsApp() {
    if (!validDetails || isSending) return;
    setIsSending(true);
  ...
  return <>
    {open && <div className="smart-modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) close(); }}>
      <div ref={dialogRef} tabIndex={-1} className="smart-modal" role="dialog" aria-modal="true" aria-labelledby="smart-modal-title">
        ...
        <div key={step} className="smart-modal-body smart-step-transition">
          {step === 1 && <div className="smart-modal-step"><p className="smart-modal-question">Hangi cihaz için destek istiyorsunuz?</p><div className={`smart-device-grid ${attempted && !effectiveDevice ? "has-error" : ""}`}>{devices.map(({ name, Icon }) => <button key={name} type="button" className={device === name ? "is-selected" : ""} aria-pressed={device === name} onClick={() => selectDevice(name)}><Icon size={20} /><span>{name}</span></button>)}</div>{attempted && !effectiveDevice && <p id="device-error" className="smart-validation" role="alert">Devam etmek için cihaz türünü yazın.</p>}{device === "Diğer" && <label htmlFor="modal-other-device">Cihaz türünü yazın<input id="modal-other-device" required aria-invalid={attempted && !effectiveDevice} aria-describedby={attempted && !effectiveDevice ? "device-error" : undefined} value={otherDevice} onChange={(event) => setOtherDevice(event.target.value)} placeholder="Örn. Robot süpürge" /></label>}</div>}
          {step === 2 && <div className="smart-modal-step"><p className="smart-modal-question">{effectiveDevice} hangi belirtiyi gösteriyor?</p><div className={`smart-symptom-grid ${attempted && !effectiveSymptom ? "has-error" : ""}`}>{[...symptoms[device], "Diğer"].map((item, index) => <button key={`${item}-${index}`} type="button" className={symptom === item ? "is-selected" : ""} aria-pressed={symptom === item} onClick={() => setSymptom(item)}><span>{String(index + 1).padStart(2, "0")}</span>{item}</button>)}</div>{attempted && !effectiveSymptom && <p id="symptom-error" className="smart-validation" role="alert">Devam etmek için arızayı kısaca yazın.</p>}{symptom === "Diğer" && <label htmlFor="modal-other-symptom">Arızayı kısaca yazın<input id="modal-other-symptom" required aria-invalid={attempted && !effectiveSymptom} aria-describedby={attempted && !effectiveSymptom ? "symptom-error" : undefined} value={otherSymptom} onChange={(event) => setOtherSymptom(event.target.value)} placeholder="Örn. Cihaz çalışırken hata veriyor" /></label>}</div>}
        ...
        <footer className="smart-modal-footer"><button type="button" className="button button-ghost dark-ghost" onClick={back}><ArrowLeft size={16} /> {step === 1 ? "Kapat" : "Geri"}</button>{step < 4 ? <button type="button" className="button button-primary" onClick={next}>Devam et <ArrowRight size={16} /></button> : <button type="button" className="button button-primary" disabled={!canContinue || isSending} onClick={sendWhatsApp}><MessageCircle size={17} />{isSending ? "WhatsApp açılıyor…" : "WhatsApp’tan bilgi ver"}</button>}</footer>
      </div>
    </div>}
  </>;
}
...
export function SmartInfoSection() {
  const openForm = () => window.dispatchEvent(new Event("esli:open-smart-info"));
  return <section className="diagnostic-section smart-info-section" aria-labelledby="diagnostic-title"><Gauge className="smart-info-watermark" aria-hidden="true" strokeWidth={1.1} />
    <div className="diagnostic-intro">
      ...
      <button type="button" className="button button-primary smart-info-launch smart-info-whatsapp" onClick={openForm} aria-label="WhatsApp üzerinden hızlı servis talebi oluştur"><span className="smart-whatsapp-mark" aria-hidden="true"><MessageCircle size={20} /><Phone size={9} /></span> WhatsApp üzerinden hızlı servis talebi oluştur</button>
    ...
    <div className="smart-info-preview" aria-label="Ön bilgi formu adımları">
      ...
      <div className="smart-preview-footer"><span><MessageCircle size={15} /> WhatsApp mesajınız hazır</span><button type="button" className="text-link" onClick={openForm}>Servis talebi oluştur <ArrowRight size={16} /></button></div>
    </div></section>;
}
