...
describe("Instagram feed", () => {
  ...
  it("returns twelve posts and uses a video thumbnail instead of an MP4", async () => {
    const video = { ...photo("video"), media_type: "VIDEO", media_url: "https://example.com/video.mp4", thumbnail_url: "https://example.com/cover.jpg" };
    const upstream = vi.fn().mockResolvedValue(Response.json({ data: [video, ...Array.from({ length: 13 }, (_, i) => photo(String(i)))] }));
