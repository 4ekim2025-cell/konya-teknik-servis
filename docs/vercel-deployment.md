...
Doğru yapılandırmada JSON içindeki `data` alanı en fazla on iki gönderi içerir. `instagram_not_configured` yanıtı görülürse iki Instagram ortam değişkenini ve yeni dağıtımın bu değişkenleri içerdiğini kontrol edin.
