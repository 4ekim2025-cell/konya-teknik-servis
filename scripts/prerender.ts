...
function staticContent(title: string, description: string, route: string) {
  ...
  if (service && !brand) {
    ...
    sections = `<h2>${esc(district)} Beyaz Eşya Servisi</h2><p>${esc(detail.intro)}</p><h2>${esc(district)} servis talebi</h2><p>${esc(detail.guide)}</p><h2>${esc(district)} için hizmet rehberleri</h2><p>${esc(detail.services)}</p><h2>Servis öncesi güvenlik</h2><p>${esc(detail.safety)}</p><h2>${esc(district)} hakkında sık sorulanlar</h2>${detail.faqs.map(([q,a]) => `<h3>${esc(q)}</h3><p>${esc(a)}</p>`).join("")}`;
