...
const districtDetails: Record<string, { intro: string; planning: string; services: string; safety: string; faqs: [string,string][] }> = {
  Karatay: { intro: "Karatay’da beyaz eşya ve küçük ev aletleri servis planlaması; cihaz türü, arıza belirtisi ve mahalle bilgisi alınarak yapılır.", planning: "Karatay’dan gelen taleplerde mahalle, açık adres tarifi ve uygun zaman aralığı günlük saha planına göre değerlendirilir.", services: "Çamaşır ve bulaşık makineleri, buzdolapları, fırın-ocaklar, davlumbaz, derin dondurucu ve su sebili servis rehberlerinden yararlanabilirsiniz.", safety: "Yanık kokusu, kaçak veya sigorta attırma varsa cihazı kullanmayın; gövdeyi ve elektrik aksamını açmayın.", faqs: [["Karatay’da hangi mahallelere servis planlanır?", "Karatay’daki servis planı açık adres ve günlük saha uygunluğuna göre yapılır. Talep sırasında mahalle ve mümkünse yakın konum bilgisini paylaşın."], ["Karatay’da aynı gün servis mümkün mü?", "Günlük ekip planı ve adres yoğunluğuna bağlı olarak aynı gün için uygunluk kontrol edilir; kesin zaman talep sırasında paylaşılır."]] },
  Meram: { intro: "Meram’da ev tipi beyaz eşya ve küçük ev aletleri için servis planı; cihaz ve adres bilgileriniz alınarak günlük uygunluğa göre netleştirilir.", planning: "Meram’dan servis talebi oluştururken cihaz türünü, marka-model bilgisini, belirtiyi ve mahalle bilgisini paylaşmanız planlamayı kolaylaştırır.", services: "Buzdolabında soğutmama, çamaşır makinesinde su alma veya sıkma, bulaşık makinesinde tahliye ve fırında ısıtmama gibi cihaz türüne özel rehberleri inceleyebilirsiniz.", safety: "Su kaçağı, duman, yanık kokusu veya elektrik riski varsa cihazı kullanmayı durdurun ve güvenli müdahale için servis desteği alın.", faqs: [["Meram’da servis için hangi adres bilgisi gerekli?", "Mahalle adı, mümkünse cadde veya sokak ve açık adres tarifi servis planlaması için yeterlidir."], ["Meram’da aynı gün servis garantisi var mı?", "Aynı gün uygunluğu günlük ekip planına ve adres yoğunluğuna göre kontrol edilir; kesin süre taahhüt edilmez."]] },
  Selçuklu: { intro: "Selçuklu’nun farklı mahallelerinden gelen beyaz eşya servis talepleri; cihaz, adres ve ekip uygunluğu birlikte değerlendirilerek planlanır.", planning: "Selçuklu’dan gelen taleplerde mahalle ve açık adres bilgisi, günlük saha planına uygun zaman aralığının belirlenmesine yardımcı olur.", services: "Çamaşır, bulaşık ve kurutma makineleri; buzdolabı, derin dondurucu, fırın, ocak ve davlumbaz için ilgili cihaz rehberlerini inceleyebilirsiniz.", safety: "Fişi çekmeden cihaz gövdesini açmayın; gaz, su, elektrik veya soğutma devresine müdahale etmeyin.", faqs: [["Selçuklu’da hangi mahallelere servis planlanır?", "Selçuklu’daki talepler mahalle, açık adres ve günlük saha planına göre değerlendirilir. Talep sırasında konum bilgisini paylaşın."], ["Selçuklu’da servis süresi nasıl belirlenir?", "Cihaz türü, arızanın niteliği, adres ve ekip uygunluğu birlikte değerlendirilir; uygun zaman talep sırasında bildirilir."]] },
};
...
function staticContent(title: string, description: string, route: string) {
  ...
  if (service && !brand) {
    ...
    sections = `<h2>${esc(district)} Beyaz Eşya Servisi</h2><p>${esc(detail.intro)}</p><h2>${esc(district)}’da servis planlaması</h2><p>${esc(detail.planning)} Cihazın markası, modeli, arıza belirtisi ve mahalle bilgisi ilk yönlendirmeyi kolaylaştırır.</p><h2>${esc(district)}’da sık talep edilen hizmetler</h2><p>${esc(detail.services)}</p><h2>Güvenli servis öncesi yaklaşım</h2><p>${esc(detail.safety)}</p><h2>${esc(district)} için sık sorulan sorular</h2>${detail.faqs.map(([q,a]) => `<h3>${esc(q)}</h3><p>${esc(a)}</p>`).join("")}`;
